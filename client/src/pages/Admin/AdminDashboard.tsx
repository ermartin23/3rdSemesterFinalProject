...
function PlayersTab() {
  ...
  function handleEditPlayer(e: React.FormEvent) {
    ...
    setPlayers((prev) =>
      prev.map((p) => (p.id === selectedPlayer.id ? updated : p))
    );
      ...
      <div className="bg-white border rounded-xl shadow p-6 text-gray-600">
        {players.length === 0 ? (
          <p className="text-center">
            No players registered yet. Click <strong>“Add Player”</strong> to
            get started.
          </p>
        ) : (
          <ul className="space-y-3">
            {players.map((player) => (
              <li
                key={player.id}
                className="p-3 bg-[#faf6ef] rounded-xl shadow flex justify-between items-center"
              >
                <span>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    className="btn btn-sm btn-outline border-red-600 text-red-600 hover:bg-red-50"
                    onClick={() => {
                      setSelectedPlayer(player);
        ...
        <Modal onClose={() => setShowEditModal(false)}>
          ...
          <form onSubmit={handleEditPlayer} className="space-y-4">
            ...
            <Input
              name="phone"
              label="Phone"
              defaultValue={selectedPlayer.phone}
            />
            ...
            <button
              type="submit"
              className="btn bg-blue-600 text-white hover:bg-blue-700 w-full mt-2"
            >
              Save Changes
            </button>
          </form>
...
function TransactionsTab() {
  return (
    <div className="max-w-5xl mx-auto mt-10">
      ...
      <div className="bg-white border rounded-xl shadow p-6 mb-10 text-center text-gray-600">
          ...
          {history.length === 0 ? (
            ...
            <ul className="space-y-3">
              {history.map((g) => (
                ...
                >
                  <span>
                    <strong>Week {g.week}</strong> — Winners:{" "}
